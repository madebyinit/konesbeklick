            ...
            <?php foreach($offers as $offer_key => $offer): ?>
               <li class="auction-bidding-history--offer <?php echo (count($offers) == $offer_key + 1 ? ' is-highest': ''); ?>" data-bid-id="<?php echo $offer['id']; ?>" style="display: none;">
                   ...
                   <?php
                       ...
                       $offer_type = $offer['proxy'] == 0 ? ($offer['userid'] == $user_id ? 'ההצעה שלי ' : (count($offers) == $offer_key + 1 ? ' הצעה מובילה': ' הצעה מתחרה')) : ($offer['proxy'] == 1 ? ($is_manager ? 'אני' : 'הקפצה מהאינטרנט') : ($is_manager ? 'אני' : 'הקפצה מהאולם'));
                   ?>
                   ...
                   <div>
                       ...
                       <small>
                           <?php
                           var_dump(get_user_meta( $offer['userid'], 'first_name', true ));
